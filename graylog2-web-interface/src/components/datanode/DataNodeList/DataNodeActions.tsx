...
const DataNodeActions = ({ dataNode, refetch, displayAs }: Props) => {
  ...
  return (
    <>
      {displayAs === 'dropdown' && (
        <MoreActions>
          <MenuItem onSelect={() => renewDatanodeCertificate(dataNode.node_id)}>续订证书</MenuItem>
          ...
          {(!isDatanodeRemoved || isRemovingDatanode) && <MenuItem onSelect={() => handleAction(DIALOG_TYPES.REMOVE)}>移除</MenuItem>}
          <MenuItem onSelect={() => setShowLogsDialog(true)}>显示日志</MenuItem>
        </MoreActions>
      )}
      {displayAs === 'buttons' && (
        <>
          {!isDatanodeRunning && <ActionButton onClick={handleStartDatanode} bsSize="small">开始</ActionButton>}
          {isDatanodeRunning && <ActionButton onClick={() => handleAction(DIALOG_TYPES.STOP)} bsSize="small">停止</ActionButton>}
          {isDatanodeRemoved && <ActionButton onClick={() => handleAction(DIALOG_TYPES.REJOIN)} bsSize="small">重新加入</ActionButton>}
          {(!isDatanodeRemoved || isRemovingDatanode) && <ActionButton onClick={() => handleAction(DIALOG_TYPES.REMOVE)} bsSize="small">移除</ActionButton>}
          <ActionButton onClick={() => setShowLogsDialog(true)} bsSize="small">显示日志</ActionButton>
        </>
      )}
      {showConfirmDialog && (
