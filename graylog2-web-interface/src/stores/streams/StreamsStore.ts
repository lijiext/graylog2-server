  ...
  Reflux.createStore({
    ...
    listStreams() {
      ...
      const promise = fetch('GET', qualifyUrl(url))
        ...
        .catch((errorThrown) => {
          UserNotification.error(`加载数据流失败，状态为：${errorThrown}`, '无法加载数据流');
        });
    ...
    pause(streamId: string, callback: () => void) {
      const failCallback = (errorThrown) => {
        UserNotification.error(`暂停数据流失败，状态为：${errorThrown}`, '无法暂停数据流');
      };
    ...
    resume(streamId: string, callback: () => void) {
      const failCallback = (errorThrown) => {
        UserNotification.error(`恢复数据流失败，状态为：${errorThrown}`, '无法恢复数据流');
      };
    ...
    update(streamId: string, data: any, callback: (stream: Stream) => void) {
      const failCallback = (errorThrown) => {
        UserNotification.error(`更新数据流失败，状态为：${errorThrown}`, '无法更新数据流');
      };
    ...
    cloneStream(streamId: string, data: any, callback: (streamId: string) => void) {
      const failCallback = (errorThrown) => {
        UserNotification.error(`克隆数据流失败，状态为：${errorThrown}`, '无法克隆数据流');
      };
