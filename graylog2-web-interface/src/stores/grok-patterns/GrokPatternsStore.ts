  ...
  () => Reflux.createStore({
    ...
    loadPatterns(callback: (patterns: Array<GrokPattern>) => void) {
      const failCallback = (error) => {
        UserNotification.error(`加载 Grok 模式失败，状态：${error.message}`,
          '无法加载 Grok 模式');
      };
    ...
    searchPaginated(page, perPage, query) {
      ...
      return fetch('GET', qualifyUrl(url))
        ...
        .catch((errorThrown) => {
          UserNotification.error(`加载模式失败，状态：${errorThrown}`,
            '无法加载数据流');
        });
    },
    ...
    savePattern(pattern: GrokPattern, callback: () => void) {
      const failCallback = (error) => {
        ...
        UserNotification.error(`测试 Grok 模式 "${pattern.name}" 失败，状态：${errorMessage}`,
          '无法测试 Grok 模式');
      };
    ...
    deletePattern(pattern: GrokPattern, callback: () => void) {
      const failCallback = (error) => {
        UserNotification.error(`删除 Grok 模式 "${pattern.name}" 失败，状态：${error.message}`,
          '无法删除 Grok 模式');
      };
    ...
    bulkImport(patterns: string, importStrategy: string) {
      const failCallback = (error) => {
        ...
        UserNotification.error(`导入 Grok 模式文件失败，状态：${errorMessage}`,
          '无法加载 Grok 模式');
      };
