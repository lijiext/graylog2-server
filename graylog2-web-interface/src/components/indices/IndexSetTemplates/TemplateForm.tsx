...
const RotationConfig = ({
  ...
  return (
    <IndexMaintenanceStrategiesConfiguration
      ...
      name="rotation"
      label="轮转策略"
      description="多个索引用于存储文档。您可以配置其用于确定何时轮换当前活动的写入索引的策略。"
                ...
                <FlexWrapper>
                  ...
                  <Section title="详情">
                    ...
                    <FormikInput
                      ...
                      help="此索引集中每个索引使用的搜索集群副本分片数。添加副本分片可提升索引并行读取（如仪表盘场景）时的搜索性能，是高可用和备份策略的组成部分。每个副本分片组会成倍增加索引的存储需求和内存占用。此值不应超过搜索节点数，通常也不应超过 1。"
                    />
                    <FormikInput
                    ...
                    <Field name="index_set_config.field_type_refresh_interval">
                      {({ field: { name, value, onChange } }) => (
                        <TimeUnitInput
                          ...
                          help={
                            <>
                              <em>高级选项。</em> 活动写入索引的字段类型信息将多久更新一次。将此值设置得更高可以略微减少搜索集群的开销并提高性能，但会导致新数据消息的搜索时间变长
                              {productName}.
                            </>
                          }
