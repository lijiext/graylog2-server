...
const useProfileMutation = () => {
  ...
  const post = useMutation(postProfile, {
    onError: (errorThrown) => {
      UserNotification.error(`创建索引集字段类型配置文件失败，状态：${errorThrown}`,
        '无法创建索引集字段类型配置文件');
    },
    onSuccess: () => {
  ...
  const put = useMutation(putProfile, {
    onError: (errorThrown) => {
      UserNotification.error(`更新索引集字段类型配置文件失败，状态：${errorThrown}`,
        '无法更新索引集字段类型配置文件');
    },
    onSuccess: () => {
  ...
  const remove = useMutation(deleteProfile, {
    onError: (errorThrown) => {
      UserNotification.error(`删除索引集字段类型配置文件失败，状态：${errorThrown}`,
        '无法删除索引集字段类型配置文件');
    },
    onSuccess: () => {
