...
public class LoggersResource extends RestResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 404, message = "不存在该子系统。")
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 404, message = "内存附加器已禁用。"),
            @ApiResponse(code = 500, message = "内存附加器已损坏。")
    })
    @Path("/messages/recent")
    @Produces(MediaType.TEXT_PLAIN)
    @RequiresPermissions(RestPermissions.LOGGERSMESSAGES_READ)
    @HideOnCloud
    public Response messages(@ApiParam(name = "limit", value = "应返回多少条日志消息。0 表示返回所有现有消息。" +
