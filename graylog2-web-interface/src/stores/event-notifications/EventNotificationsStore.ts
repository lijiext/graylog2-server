      ...
      promise.catch((error) => {
        if (error.status === 404) {
          UserNotification.error(
            `找不到 ID 为 <${notificationId}> 的事件通知，请确保其未被删除。`,
            '无法检索事件通知',
          );
        }
      });
      ...
      promise.then(
        ...
        (error) => {
          if (error.status !== 400 || !error.additional.body || !error.additional.body.failed) {
            UserNotification.error(
              `创建通知 "${notification.title}" 失败，状态为：${error}`,
              '无法保存通知',
            );
          }
        },
    ...
    update(notificationId, notification) {
      ...
      promise.then(
        ...
        (error) => {
          if (error.status !== 400 || !error.additional.body || !error.additional.body.failed) {
            UserNotification.error(
              `更新通知 "${notification.title}" 失败，状态为：${error}`,
              '无法更新通知',
            );
          }
        },
    ...
    delete(notification) {
      ...
      promise.then(
        () => {
          UserNotification.success(
            '通知删除成功',
            `通知 "${notification.title}" 已成功删除。`,
          );
          this.refresh();
        },
        (error) => {
          UserNotification.error(
            `删除通知 "${notification.title}" 失败，状态为：${error}`,
            '无法删除通知',
          );
        },
      );
