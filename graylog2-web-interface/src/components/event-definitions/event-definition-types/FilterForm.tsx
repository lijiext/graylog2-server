...
const StreamCategorySelector = ({ onChange, streams, value }: StreamCategorySelectorProps) => {
  ...
  return (
    <FormGroup controlId="filter-stream-categories">
      <ControlLabel>
        数据流类别 <small className="text-muted">（可选）</small>
      </ControlLabel>
...
const QueryParameters = ({ eventDefinition, onChange, userCanViewLookupTables, validation }: QueryParametersProps) => {
  ...
  if (!userCanViewLookupTables) {
    return <Alert bsStyle="info">此账户缺少权限声明来自查找表的查询参数。</Alert>;
  }
    ...
    <fieldset>
      ...
      {onlyFilters || (
        <Input
          ...
          help={
            <span>
              消息应匹配的搜索查询。您可以使用与搜索页面相同的语法，包括通过使用查找表声明查询参数 <code>$newParameter$</code> 语法。
            </span>
          }
      ...
      {!searchFiltersHidden && (
        <FormGroup controlId="search-filters">
          <ControlLabel>
            搜索过滤器 <small className="text-muted">（可选）</small>
          </ControlLabel>
      ...
      {onlyFilters || (
        <>
          <FormGroup controlId="filter-streams">
            <ControlLabel>数据流{!isStreamRequired && <small className="text-muted"> （可选）</small>}</ControlLabel>
            <MultiSelect
              id="filter-streams"
          ...
          {isSearchingWarmTier(warmTierRanges) && (
            <Alert bsStyle="danger" title="热存储层警告">
              所选时间范围将包含存储在温层中的数据。必须频繁从温层检索数据的事件可能会导致性能问题。一个值{' '}
              <strong>搜索最近</strong> 超过以下时长将进入温存储层:{' '}
              <WarmTierTimeStamp />.
            </Alert>
          )}
