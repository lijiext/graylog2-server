...
public class IndicesResource extends RestResource {
    ...
    @Timed
    @Path("/{index}")
    @ApiOperation(value = "获取索引及其分片的信息。")
    ...
    @Path("/open")
    @Timed
    @ApiOperation(value = "获取 Graylog 管理的所有打开的索引及其分片的信息。")
    ...
    @Timed
    @Path("/reopened")
    @ApiOperation(value = "获取已重新打开的索引列表，这些索引不会被保留策略清理")
    @Produces(MediaType.APPLICATION_JSON)
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @ApiResponses(value = {
            @ApiResponse(code = 403, message = "您无法关闭当前的偏转器目标索引。")
    })
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @ApiResponses(value = {
            @ApiResponse(code = 403, message = "您无法删除当前的偏转器目标索引。")
    })
