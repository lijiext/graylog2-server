...
public class StreamRuleResource extends RestResource {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 400, message = "无效的 JSON 正文。")
    })
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public StreamRuleListResponse get(@ApiParam(name = "streamid", value = "我们想要其流规则的流 ID。", required = true)
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public StreamRule get(@ApiParam(name = "streamid", value = "我们想要其流规则的流 ID。", required = true) @PathParam("streamid") String streamid,
