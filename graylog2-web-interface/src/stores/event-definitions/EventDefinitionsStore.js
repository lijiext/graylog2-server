  ...
  () => Reflux.createStore({
    ...
    listPaginated({ query = '', page = 1, pageSize = 10 }) {
      ...
      promise.then((response) => {
        ...
        UserNotification.error(`获取事件定义失败，状态码：${error}`,
          '无法检索事件定义');
      });
    ...
    get(eventDefinitionId) {
      ...
      promise.catch((error) => {
        if (error.status === 404) {
          UserNotification.error(`找不到 ID 为 <${eventDefinitionId}> 的事件定义，请确保其未被删除。`,
            '无法检索事件定义');
        }
      });
      ...
      promise.then(
        ...
        (error) => {
          if (error.status !== 400 || !error.additional.body || !error.additional.body.failed) {
            UserNotification.error(`创建事件定义 "${eventDefinition.title}" 失败，状态码：${error}`,
              '无法保存事件定义');
          }
        },
      );
    ...
    copy(eventDefinition) {
      ...
      promise.then(
        (response) => {
          UserNotification.success('事件定义复制成功',
            `事件定义“${response.title}”创建成功。`);
        ...
        (error) => {
          if (error.status !== 400 || !error.additional.body || !error.additional.body.failed) {
            UserNotification.error(`复制事件定义 "${eventDefinition.title}" 失败，状态码：${error}`,
              '无法复制事件定义');
          }
        },
      );
    ...
    update(eventDefinitionId, updatedEventDefinition) {
      ...
      promise.then(
        ...
        (error) => {
          if (error.status !== 400 || !error.additional.body || !error.additional.body.failed) {
            UserNotification.error(`更新事件定义 "${eventDefinition.title}" 失败，状态为：${error}`,
              '无法更新事件定义');
          }
        },
      );
    ...
    enable(eventDefinition) {
      ...
      promise.then(
        (response) => {
          UserNotification.success('事件定义已成功启用',
    ...
    clearNotificationQueue(eventDefinition) {
      ...
      promise.then(
        (response) => {
          UserNotification.success('已清除待处理的通知。',
            '排队的通知已成功清除。');
        ...
        (error) => {
          if (error.status !== 400 || !error.additional.body || !error.additional.body.failed) {
            UserNotification.error(`清除待处理的通知失败，状态为：${error}`,
              '无法清除待处理的告警');
          }
        },
      );
