...
then
  ...
  return (
    <div>
      ...
      <h5 style={{ marginBottom: 10 }}>查找 URL</h5>
      <p style={{ marginBottom: 10, padding: 0 }}>
        将用于 HTTP 请求的 URL。若要使用 <em>查找键</em> 在 URL 中，
        <code>{'${key}'}</code>
        值可被使用。此变量将被替换为传递给查找函数的实际键。 <br />
        (示例： <code>{'https://example.com/api/lookup?key=${key}'}</code>)
      </p>

      <h5 style={{ marginBottom: 10 }}>单值 JSONPath</h5>
      <p style={{ marginBottom: 10, padding: 0 }}>
        此 JSONPath 表达式将用于解析 <em>单个值</em> 查找结果的一部分。（示例： <code>$.user.full_name</code>)
      </p>

      <h5 style={{ marginBottom: 10 }}>多值 JSONPath</h5>
      <p style={{ marginBottom: 10, padding: 0 }}>
        此 JSONPath 表达式将用于解析 <em>多值</em> 查找结果的一部分。（示例： <code>$.users[*]</code>) 多值 JSONPath 设置是 <em>optional</em>。如果没有它，单个值也会出现在多值结果中。
      </p>

      <h5 style={{ marginBottom: 10 }}>HTTP User-Agent</h5>
      <p style={{ marginBottom: 10, padding: 0 }}>
        这是 <em>用户代理</em> 将用于 HTTP 请求的标头。您应包含一些联系信息，以便您查询的服务所有者在出现问题时知道联系谁（例如来自您的 Graylog 集群的过多 API 请求）。
      </p>
      ...
      <p>
        此示例显示配置以及从查找表返回的值。<br />
        配置的 URL 是 <strong>{'https://example.com/api/users/${key}'}</strong> 以及 <code>{'${key}'}</code>
      ...
      <Row>
        <Col md={4}>
          ...
          <p style={{ marginBottom: 10, padding: 0 }}>
            单个值 JSONPath: <code>$.user.full_name</code><br />
      ...
      <Row>
        <Col md={4}>
          <h5 style={{ marginBottom: 10 }}>配置</h5>
          <p style={{ marginBottom: 10, padding: 0 }}>
            单个值 JSONPath: <code>$.user.contact.email</code><br />
