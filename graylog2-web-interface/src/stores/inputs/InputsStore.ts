  ...
  Reflux.createStore<InputsStoreState>({
    ...
    list() {
      ...
      promise.then(
        ...
        (error) => {
          UserNotification.error(`获取输入失败，状态为：${error}`, '无法检索输入端');
        },
      );
    ...
    delete(input) {
      ...
      promise.then(
        () => {
          UserNotification.success(`输入 '${inputTitle}' 已成功删除`);
          InputsActions.list();
        },
        (error) => {
          UserNotification.error(
            `删除输入 '${inputTitle}' 失败，状态为：${error}`,
            '无法删除输入端',
          );
        },
      );
