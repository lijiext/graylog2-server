...
public class ClusterSystemResource extends ProxiedResource {
    ...
    @Path("{nodeId}/jvm")
    public SystemJVMResponse jvm(@ApiParam(name = "nodeId", value = "检索 JVM 信息的节点 ID。", required = true)
    ...
    public SystemProcessBufferDumpResponse processBufferDump(@ApiParam(name = "nodeId", value = "获取进程缓冲区转储的节点 ID。", required = true)
                                                             @PathParam("nodeId") String nodeId) throws IOException, NodeNotFoundException {
        final Node targetNode = nodeService.byNodeId(nodeId);
