...
const UrlWhiteListForm = ({ urls, onUpdate, disabled, newEntryId }: Props) => {
    ...
    <tr key={url.id}>
      ...
      <td>
        <Input type="text"
               ...
               help={validationState.errors[idx] && validationState.errors[idx].title && !validationState.errors[idx].title.valid ? '必填字段' : null}
               name="title"
  ...
  return (
    <>
      ...
      <Button bsSize="sm" onClick={(event) => _onAdd(event)}>添加 Url</Button>
