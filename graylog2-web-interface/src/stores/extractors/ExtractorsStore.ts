  ...
  Reflux.createStore<ExtractorsStoreState>({
    ...
    create(inputId: string, extractor: ExtractorType, calledFromMethod: boolean) {
      ...
      promise
        ...
        .catch((error) => {
          UserNotification.error(`创建提取器失败：${error}`, '无法创建提取器');
        });
    ...
    update(inputId: string, extractor: ExtractorType, calledFromMethod: boolean) {
      ...
      promise
        ...
        .catch((error) => {
          UserNotification.error(`更新提取器失败：${error}`, '无法更新提取器');
        });
    ...
    order(inputId: string, orderedExtractors: Array<ExtractorType>) {
      ...
      promise.catch((error) => {
        UserNotification.error(`更改提取器位置失败：${error}`, '无法更新提取器位置');
      });
      ...
      Promise.allSettled(promises).then(() => {
        if (failedImports === 0) {
          UserNotification.success(
            `导入结果：已导入 ${successfulImports} 个提取器。`,
            '导入操作成功',
          );
          ...
          UserNotification.warning(
            `导入结果：已导入 ${successfulImports} 个提取器，${failedImports} 个错误。`,
            '导入操作已完成',
          );
        }
      });
